...
class GuassianDiffusion:
    ...
    def sample_from_reverse_process(
        self, model, xT, ref_x, timesteps=None, model_kwargs={}, ddim=False, sampling_only=False
    ):
        ...
        for i, t in zip(np.arange(timesteps)[::-1], new_timesteps[::-1]):
            with torch.no_grad():
                ...
                if sampling_only == False:
                    pred_epsilon = model(final, current_t, **model_kwargs)
                else:
                    if ref_x is None:
                        ref_x_noise = None
                    else:
                        ref_x_noise, _ = self.sample_from_forward_process(ref_x, current_t)
                    pred_epsilon = model(final, current_t, ref_x_noise, **model_kwargs)
...
def get_dice_loss(inputs, targets):
    smooth = 1e-5
    # inputs = F.sigmoid(inputs)
    inputs = inputs.view(1, -1)
    targets = targets.view(1, -1)
    intersection = torch.sum(inputs*targets, dim=1)
    dice = (2.*intersection+smooth)/(torch.sum(inputs, dim=1)+torch.sum(targets, dim=1)+smooth)
    return torch.mean(dice).detach().cpu().numpy()
...
def sample_N_images(
    ...
    while num_samples < N:
        if xT is None:
            xT = (
                torch.randn(batch_size, num_channels, image_size[0], image_size[1])
                .float()
                .to(args.device)
            )
        if args.class_cond:
            if indices is None:
                indices = np.random.randint(0, 157, 8)
            y = torch.from_numpy(possible_y[indices][:, :num_classes]).to(args.device)
            for idx in indices:
                if idx != 156:
                    path = os.path.join(
                    'data', "mask_img", "".join(["0"] * (5 - len(str(idx)))) + str(idx) + ".png",
                    )
        ...
        if args.sampling_only:
            gen_images = diffusion.sample_from_reverse_process(
                model, xT, ref_x, sampling_steps, {"y": y}, args.ddim, args.sampling_only
            )
        else:
            ref_x = False
            gen_images = diffusion.sample_from_reverse_process(
        ...
        gt_target = torch.from_numpy(np.array(gt.copy())).to(args.device)
        gt_target = gt_target/255.0
        gt_target[gt_target>=0.5] = 1
        gt_target[gt_target<0.5] = 0
        gt_target = 1-gt_target
        inputs = (127.5 * (gen_images.clone().squeeze() + 1))/255.0
        inputs[inputs>=0.5] = 1
        inputs[inputs<0.5] = 0
        inputs = 1-inputs
        dice = get_dice_loss(inputs, gt_target)
    ...
    if not args.sampling_only:
        ...
        return (samples, gt, dice)
    else:
        ori_samples=np.concatenate(samples).copy()
        samples = np.concatenate(samples).transpose(0, 2, 3, 1)[:N]
        samples = (127.5 * (samples + 1)).astype(np.uint8)
        return (samples, gt, dice, ori_samples)
...
def main():
    ...
    # misc
    parser.add_argument("--save-dir", type=str, default="./trained_models/")#"/home/jyfang/min_diffusion/trained_models")
    ...
    if args.sampling_only:
        model = AttentionInjection(model)
    
    
    
    possible_y = np.load("data/finger_dots_w_frame_label.npy")[:152]
    
    if args.sampling_only:
        dice_list = []
        for sample_id in range(157):
            if sample_id == 0:
                ref_x = None
            else:
                ref_x = torch.from_numpy(ori_samples).to(args.device)
                print(ref_x.shape)
            indices = [sample_id]
            sampled_images, gt, dice, ori_samples = sample_N_images(
                1,
                possible_y, 
                ref_x,
                model,
            ...
            dice_round = round(dice.item(), 2)
            cv2.imwrite(
                os.path.join(
                    args.save_dir,
                    f"{args.arch}_{sample_id}_0_{dice_round}_{args.dataset}-{args.diffusion_steps}_steps-{args.sampling_steps}-sampling_steps-class_condn_{args.class_cond}.png",
                ),
    ...
    ref_x = False
    sampled_images, gt, dice = sample_N_images(
            8,
            possible_y, 
            ref_x,
            model,
    ...
    if args.local_rank == 0:
        ...
        cv2.imwrite(
            os.path.join(
                args.save_dir,
                f"{args.arch}_0_{dice_round}_{args.dataset}-{args.diffusion_steps}_steps-{args.sampling_steps}-sampling_steps-class_condn_{args.class_cond}.png",
            ),
            np.concatenate(sampled_images, axis=1)[:, :, ::-1],
        )
        cv2.imwrite(
            os.path.join(
                args.save_dir,
                f"gt_{args.arch}_0_{dice_round}_{args.dataset}-{args.diffusion_steps}_steps-{args.sampling_steps}-sampling_steps-class_condn_{args.class_cond}.png",
            ),
            gt,
        )
        ...
        if not epoch % 1:
            ...
            if args.local_rank == 0:
                ...
                cv2.imwrite(
                    os.path.join(
                        args.save_dir,
                        f"{args.arch}_{epoch}_{dice_round}_{args.dataset}-{args.diffusion_steps}_steps-{args.sampling_steps}-sampling_steps-class_condn_{args.class_cond}.png",
                    ),
                    np.concatenate(sampled_images, axis=1)[:, :, ::-1],
                )
                cv2.imwrite(
                    os.path.join(
                        args.save_dir,
                        f"gt_{args.arch}_{epoch}_{dice_round}_{args.dataset}-{args.diffusion_steps}_steps-{args.sampling_steps}-sampling_steps-class_condn_{args.class_cond}.png",
                    ),
                    gt,
                )
        if args.local_rank == 0:
